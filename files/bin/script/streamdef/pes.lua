...
function pes(pid, size, out_file_name)
	...
	ofs = fstr("00 00 01", 0x7fffffff, true)
	assert(ofs == 0)
	start_code = lbyte(4)

	cbit("packet_start_code_prefix", 24, 1)
	local stream_id = rbit("stream_id", 8)
	...
	and stream_id ~= padding_stream
	and stream_id ~= private_stream_2
	and stream_id ~= ECM_stream
	and stream_id ~= EMM_stream
	and stream_id ~= program_stream_directory
	and stream_id ~= ITU_T_Rec_H_222_0_ISO_IEC_13818_1_Annex_B_or_ISO_IEC_13818_6_DSMCC_stream
	and stream_id ~= ITU_T_Rec_H_222_1_type_E then
		rbit("'10'", 2, 2)
		...
		if get("PTS_DTS_flags") & 2 == 2 then
			...
			set("PTS[90kHz](sid="..hexstr(stream_id)..")", hexstr(PTS))
			set("PTS[ms](sid="..hexstr(stream_id)..")", decstr(PTS/90000))
		...
		if get("PTS_DTS_flags") & 1 == 1 then
			...
			set("DTS[90kHz](sid="..hexstr(stream_id)..")", hexstr(DTS))
			set("DTS[ms](sid="..hexstr(stream_id)..")", decstr(DTS/90000))
	    end
		...
		if no_packet_length then
			if size ~= nil then
				...
				seek(cur()+4)
				local ofs = fstr(val2str(start_code), 0x7fffffff, false)
				seek(cur()-4)
				if ofs ~= false then
	...
	or     stream_id == private_stream_2
	or     stream_id == ECM_stream
	or     stream_id == EMM_stream
	or     stream_id == program_stream_directory
	or     stream_id == ITU_T_Rec_H_222_0_ISO_IEC_13818_1_Annex_B_or_ISO_IEC_13818_6_DSMCC_stream
	or     stream_id == ITU_T_Rec_H_222_1_type_E then
...
function pes_stream(size)
	fstr("00 00 01", 0x7fffffff, true)
	start_code = lbyte(4)
	enable_print(true)

	local total_size = 0;
	while total_size < size do
		if fstr("00 00 01", 0x10000, false) == get_size()-cur() then
			break
		end
