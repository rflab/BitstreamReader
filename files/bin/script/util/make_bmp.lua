...
function create_bmp(filename, dip)
	write(filename, "BM")
	write(filename, val2str(dip.pitch*dip.hight*3 +40, 4, true)) -- "bfSize",           4)
	...
	write(filename, val2str(dip.hight, 4, true))                 -- "biHeight",         4)
	write(filename, "01 00")                                     -- "biPlanes",         2)
	write(filename, "18 00")                                     -- "biBitCount",       2)
	write(filename, "00 00 00 00")                               -- "biCompression",    4)
	write(filename, val2str(dip.pitch*dip.hight*3, 4, true))     -- "biSizeImage",      4)
